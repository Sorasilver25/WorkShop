...
// Création de l'objet de données pour la conversion en HTML
$data = array(
    ...
    "name" => "result.html" // Nom du fichier de sortie
);

// Envoie une requête POST à l'API pour convertir en HTML
$curl = curl_init();
curl_setopt($curl, CURLOPT_URL, 'https://api.pdf.co/v1/pdf/convert/to/html');
...
// Si la conversion est réussie, ouvrez le fichier HTML converti dans un nouvel onglet
if (isset($responseData['url'])) {
    echo '<h1>Conversion réussie !</h1>';
    echo '<a href="' . $responseData['url'] . '" target="_blank">Afficher le fichier HTML converti</a>';
} else {
